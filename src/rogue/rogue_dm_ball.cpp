...
[[maybe_unused]] constexpr spawnflags_t SPAWNFLAG_DBALL_GOAL_TEAM_RED = 0x0001_spawnflag;
// unused; assumed by not being team_red
// constexpr uint32_t SPAWNFLAG_DBALL_GOAL_TEAM_BLUE	= 0x0002;

// globals

edict_t *dball_ball_entity = nullptr;
int		 dball_ball_startpt_count;
int		 dball_team_red_goalscore;
int		 dball_team_blue_goalscore;

cvar_t *dball_team_red_skin;
cvar_t *dball_team_blue_skin;
cvar_t *goallimit;
...
int DBall_CheckDMRules()
{
	...
	{
		if (dball_team_red_goalscore >= goallimit->integer)
			gi.LocBroadcast_Print(PRINT_HIGH, "Red Team wins!\n");
		else if (dball_team_blue_goalscore >= goallimit->integer)
			gi.LocBroadcast_Print(PRINT_HIGH, "Blue Team wins!\n");
		else
			return 0;
...
{
#if 0
	uint32_t 	 team_red, team_blue, unassigned;
	edict_t	    *other;
	char		*p;
	...
	for (uint32_t j = 1; j <= game.maxclients; j++)
	{
		...
		if (p)
		{
			if (!strcmp(dball_team_red_skin->string, value))
				team_red++;
			else if (!strcmp(dball_team_blue_skin->string, value))
				team_blue++;
			else
				unassigned++;
		}
	...
	if (team_red > team_blue)
	{
		...
		Info_SetValueForKey(ent->client->pers.userinfo, "skin", dball_team_blue_skin->string);
	}
	else
	{
		gi.Com_Print("assigned to team 1\n");
		Info_SetValueForKey(ent->client->pers.userinfo, "skin", dball_team_red_skin->string);
	}
...
bool DBall_SelectSpawnPoint(edict_t *ent, vec3_t &origin, vec3_t &angles, bool force_spawn)
{
	...
	if (!strcmp(dball_team_red_skin->string, skin))
		spottype = "dm_dball_team1_start";
	else if (!strcmp(dball_team_blue_skin->string, skin))
...
void DBall_GameInit()
{
	...
	dball_team_red_goalscore = 0;
	dball_team_blue_goalscore = 0;
	...
	dball_team_red_skin = gi.cvar("dball_team_red_skin", "male/ctf_r", CVAR_NOFLAGS);
	dball_team_blue_skin = gi.cvar("dball_team_blue_skin", "male/ctf_b", CVAR_NOFLAGS);
...
TOUCH(DBall_GoalTouch) (edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self) -> void
{
	...
	if (self->spawnflags.has(SPAWNFLAG_DBALL_GOAL_TEAM_RED))
	{
		dball_team_red_goalscore += (int) self->wait;
		team_score = 1;
	}
	else
	{
		dball_team_blue_goalscore += (int) self->wait;
		team_score = 2;
	}
	...
	for (uint32_t j = 1; j <= game.maxclients; j++)
	{
		...
		if (p)
		{
			...
			{
				if (team_score == 1)
					G_AdjustPlayerScore(ent->client, scoreChange, false, 0);
...
/*QUAKED dm_dball_goal (1 .5 .5) ? TEAM_RED TEAM_BLUE
Deathball goal
