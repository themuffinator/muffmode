...
void Tag_Score(edict_t *attacker, edict_t *victim, int scoreChange, const mod_t &mod)
{
	...
	if (tag_token && tag_owner)
	{
		...
		if ((scoreChange > 0) && tag_owner == attacker)
		{
			...
			if (tag_count == 5)
			{
				quad = GetItemByIndex(IT_POWERUP_QUAD);
				attacker->client->pers.inventory[IT_POWERUP_QUAD]++;
	...
	G_AdjustPlayerScore(attacker->client, scoreChange, false, 0);
}
...
THINK(Tag_Respawn) (edict_t *ent) -> void
{
	...
	spot = SelectDeathmatchSpawnPoint(vec3_origin, true, false, true, false).spot;
	if (spot == nullptr)
	{
