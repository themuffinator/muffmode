...
Serialize the player's session configuration to disk for persistence.
=============
*/
void PCfg_WriteConfig(gentity_t *ent) {
	...
	char sanitized_social_id[MAX_INFO_VALUE];
	Q_strlcpy(sanitized_social_id, ent->client->pers.social_id, sizeof(sanitized_social_id));
	for (char *c = sanitized_social_id; *c; ++c) {
		if (!std::isalnum(static_cast<unsigned char>(*c)) && *c != '_' && *c != '-')
			*c = '_';
	}

	const std::string path = std::string(G_Fmt("baseq2/pcfg/{}.cfg", sanitized_social_id));
	...
	string += G_Fmt("killbeep_num {}\n", static_cast<int>(ent->client->sess.pc.killbeep_num)).data();
	string += G_Fmt("follow_killer {}\n", ent->client->sess.pc.follow_killer ? 1 : 0).data();
	string += G_Fmt("follow_leader {}\n", ent->client->sess.pc.follow_leader ? 1 : 0).data();
	string += G_Fmt("follow_powerup {}\n", ent->client->sess.pc.follow_powerup ? 1 : 0).data();
	string += G_Fmt("use_expanded {}\n", ent->client->sess.pc.use_expanded ? 1 : 0).data();

	const size_t expected = string.length();
	const size_t written = std::fwrite(string.data(), 1, expected, f);
	if (std::fclose(f) != 0 || written != expected) {
		gi.Com_PrintFmt("{}: Failed to write full player config to \"{}\" ({} of {} bytes)\n", __FUNCTION__, path, written, expected);
		return;
	}
