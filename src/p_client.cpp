	...
	if (attacker && attacker->client) {
		...
		if (deathmatch->integer && g_frag_messages->integer && self != attacker && self->client && attacker->client) {
			if (!(self->svflags & SVF_BOT)) {
				gi.LocClient_Print(self, PRINT_CENTER, "You were {} by {}", freeze->integer ? "frozen" : "fragged", attacker->client->pers.netname);
			}
		...
		if (clanarena->integer) {
			...
			for (auto player : active_players()) {
				if (player == ent || !player->client->pers.spawned ||
					ClientIsSpectating(player->client) || player->movetype == MOVETYPE_NOCLIP)
					continue;
...
static edict_t *SelectCTFSpawnPoint(edict_t *ent, bool force_spawn) {
	if (ent->client->resp.ctf_state) {
		select_spawn_result_t result = SelectDeathmatchSpawnPoint(ent->client->spawn_origin, g_dm_spawn_farthest->integer, force_spawn, false, false);	// ClientIsSpectating(ent->client));
	...
	if (!spawn_points.size()) {
		select_spawn_result_t result = SelectDeathmatchSpawnPoint(ent->client->spawn_origin, g_dm_spawn_farthest->integer, force_spawn, true, false);	// ClientIsSpectating(ent->client));
...
static bool InitPlayerTeam(edict_t *ent) {
	...
	if (!g_dm_force_join->integer || (competition->integer && level.match >= MATCH_SETUP)) {
		// start as spectator
...
void P_Match_AssignGhost(edict_t *ent) {
	...
	Q_strlcpy(level.ghosts[ghost].netname, ent->client->pers.netname, sizeof(level.ghosts[ghost].netname));
...
static bool ClientInactivityTimer(edict_t *ent) {
	...
	if (cv && !ent->client->resp.inactivity_time) {
		ent->client->resp.inactivity_time = level.time + cv;
		ent->client->resp.inactivity_warning = false;
		return true;
	}
	if (!deathmatch->integer || !cv || ClientIsSpectating(ent->client) || (ent->svflags & SVF_BOT) || ent->s.number == 0) {
		// give everyone some time, so if the operator sets g_inactivity during
		// gameplay, everyone isn't kicked
		ent->client->resp.inactivity_time = level.time + 1_min;
		ent->client->resp.inactivity_warning = false;
#if 0	//muff not sure if i want this
		if (ClientIsSpectating(ent->client) && ent->client->resp.inactive && ent->client->latched_buttons & BUTTON_ANY)
			ent->client->resp.inactive = false;
#endif
		...
		if (level.time > ent->client->resp.inactivity_time) {
			gi.LocClient_Print(ent, PRINT_CENTER, "You have been dropped due to inactivity.\n");
			gi.Client_Print(ent, PRINT_HIGH, "Dropped due to inactivity.\n");	//TODO: "$g_dropped_due_inactivity");
		...
		if (level.time > ent->client->resp.inactivity_time - gtime_t::from_sec(10) && !ent->client->resp.inactivity_warning) {
			...
			gi.LocClient_Print(ent, PRINT_CENTER, "Ten seconds until inactivity drop!\n");	//TODO: "$g_ten_sec_until_drop");
		}
#endif
		if (level.time > ent->client->resp.inactivity_time) {
			gi.LocClient_Print(ent, PRINT_CENTER, "You have been removed from the match\ndue to inactivity.\n");
			Team_Join(ent, TEAM_SPECTATOR, true);
	...
	if (ent->client->chase_target) {
		...
		if (ent->movetype == MOVETYPE_NOCLIP) {
			if (ent->client->menu) {
				...
				client->ps.pmove.pm_type = PM_FREEZE;
			else if (ClientIsSpectating(ent->client))
...
void ClientBeginServerFrame(edict_t *ent) {
	...
	if (!client->weapon_thunk && !ClientIsSpectating(client))
		Think_Weapon(ent);
	else
