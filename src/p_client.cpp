...
static void PCfg_ClientInitPConfig(gentity_t* ent) {
	...
	if (f != NULL) {
		...
		if (length > 0x40000) {
			cfg_valid = false;
			gi.Com_PrintFmt("{}: Player config too large ({} bytes) for \"{}\"; discarding.\n", __FUNCTION__, (int)length, name);
		}
		if (cfg_valid) {
			...
			if (length) {
				...
				if (length != read_length) {
					cfg_valid = false;
					gi.Com_PrintFmt("{}: Player config read error for \"{}\" ({} of {} bytes); discarding.\n", __FUNCTION__, name, (int)read_length, (int)length);
				}
			}
			buffer[length] = '\0';
		}
		...
		if (!cfg_valid) {
			...
			if (!std::remove(name)) {
				gi.Com_PrintFmt("{}: Removed invalid player config \"{}\".\n", __FUNCTION__, name);
			}
			else {
				FILE* trunc = fopen(name, "w");
				if (trunc) {
					fclose(trunc);
					gi.Com_PrintFmt("{}: Truncated invalid player config \"{}\".\n", __FUNCTION__, name);
				}
				else {
					gi.Com_PrintFmt("{}: Failed to remove invalid player config \"{}\".\n", __FUNCTION__, name);
				}
			}
			need_regen = true;
			file_exists = false;
