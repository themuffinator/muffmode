...
static void PCfg_ClientInitPConfig(gentity_t* ent) {
	...
	constexpr bool default_show_id = true;
	constexpr bool default_show_timer = true;
	constexpr bool default_show_fragmessages = true;
	constexpr int8_t default_killbeep = 1;

	bool show_id = default_show_id;
	bool show_timer = default_show_timer;
	bool show_fragmessages = default_show_fragmessages;
	int8_t killbeep_num = default_killbeep;
	...
	if (f != NULL) {
		...
		if (buffer) {
			for (char* line = buffer; line && *line; ) {
				char* next_line = std::strchr(line, '\n');
				if (next_line) {
					*next_line = '\0';
					++next_line;
				}
				...
				char key[MAX_QPATH] = { 0 };
				char value[MAX_QPATH] = { 0 };
				if (std::sscanf(line, "%63s %63s", key, value) != 2) {
					gi.Com_PrintFmt("{}: Malformed line in {}: {}\n", __FUNCTION__, name, line);
					line = next_line;
					continue;
				}

				if (!Q_strcasecmp(key, "show_id")) {
					char* end = nullptr;
					...
					if (end && *end == '\0' && (parsed == 0 || parsed == 1)) {
						...
						gi.Com_PrintFmt("{}: Invalid show_id value in {}: {} (using default)\n", __FUNCTION__, name, value);
						show_id = default_show_id;
					}
				}
				else if (!Q_strcasecmp(key, "show_fragmessages")) {
					char* end = nullptr;
					const long parsed = std::strtol(value, &end, 10);
					if (end && *end == '\0' && (parsed == 0 || parsed == 1)) {
						show_fragmessages = parsed != 0;
					}
					else {
						gi.Com_PrintFmt("{}: Invalid show_fragmessages value in {}: {} (using default)\n", __FUNCTION__, name, value);
						show_fragmessages = default_show_fragmessages;
					}
				}
				else if (!Q_strcasecmp(key, "show_timer")) {
					char* end = nullptr;
					const long parsed = std::strtol(value, &end, 10);
					if (end && *end == '\0' && (parsed == 0 || parsed == 1)) {
						show_timer = parsed != 0;
						...
						gi.Com_PrintFmt("{}: Invalid show_timer value in {}: {} (using default)\n", __FUNCTION__, name, value);
						show_timer = default_show_timer;
					}
				}
				else if (!Q_strcasecmp(key, "killbeep_num")) {
					char* end = nullptr;
					const long parsed = std::strtol(value, &end, 10);
					if (end && *end == '\0' && parsed >= 0 && parsed <= 4) {
						killbeep_num = static_cast<int8_t>(parsed);
					}
					else {
						gi.Com_PrintFmt("{}: Invalid killbeep_num value in {}: {} (using default)\n", __FUNCTION__, name, value);
						killbeep_num = default_killbeep;
					}
				}
				else {
					gi.Com_PrintFmt("{}: Unknown key in {}: {}\n", __FUNCTION__, name, key);
				}
	...
	ent->client->sess.pc.show_id = show_id;
	ent->client->sess.pc.show_fragmessages = show_fragmessages;
	ent->client->sess.pc.show_timer = show_timer;
	ent->client->sess.pc.killbeep_num = killbeep_num;
}
//=======================================================================
struct mon_name_t {
