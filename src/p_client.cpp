...
static void PCfg_ClientInitPConfig(gentity_t* ent) {
	...
	const std::string directory = "baseq2/pcfg";
	std::error_code dir_error;
	if (!std::filesystem::create_directories(directory, dir_error) && dir_error) {
		gi.Com_PrintFmt("{}: Failed to create player config directory \"{}\": {}\n", __FUNCTION__, directory, dir_error.message().c_str());
		return;
	}
	...
	if (!file_exists) {
		...
		if (f) {
			...
			std::string defaults;
			defaults.reserve(128);
			defaults += G_Fmt("show_id {}\n", ent->client->sess.pc.show_id ? 1 : 0).data();
			defaults += G_Fmt("show_timer {}\n", ent->client->sess.pc.show_timer ? 1 : 0).data();
			defaults += G_Fmt("show_fragmessages {}\n", ent->client->sess.pc.show_fragmessages ? 1 : 0).data();
			defaults += G_Fmt("killbeep_num {}\n", ent->client->sess.pc.killbeep_num).data();
			defaults += G_Fmt("follow_killer {}\n", ent->client->sess.pc.follow_killer ? 1 : 0).data();
			defaults += G_Fmt("follow_leader {}\n", ent->client->sess.pc.follow_leader ? 1 : 0).data();
			defaults += G_Fmt("follow_powerup {}\n", ent->client->sess.pc.follow_powerup ? 1 : 0).data();
			defaults += G_Fmt("use_expanded {}\n", ent->client->sess.pc.use_expanded ? 1 : 0).data();

			const std::string content = header + defaults;
			const size_t written = fwrite(content.c_str(), 1, content.length(), f);
			if (written != content.length()) {
				gi.Com_PrintFmt("{}: Failed to write player config \"{}\": {}\n", __FUNCTION__, name, std::strerror(errno));
			}
			else {
				gi.Com_PrintFmt("{}: Player config written to: \"{}\"\n", __FUNCTION__, name);
			}
