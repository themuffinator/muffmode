...
// Changes Globals:		-
//========================================================================
static int ReadString(FILE *fp, char *filename, int line, char *string, int maxlen) {
	int c, i;
...
// Changes Globals:		-
//========================================================================
static int ReadSpace(FILE *fp, char *filename, int line) {
	int c;
...
// Changes Globals:		-
//========================================================================
static int LoadBotsFromFile(char *filename) {
	int line, lastline, c, numbots;
	FILE *fp;
	char addbot[MAX_QPATH];
	bot_t *bot, tmpbot;
	...
	while (!feof(fp)) {
		...
		ungetc(c, fp);
		//sv
		if (!ReadString(fp, filename, line, addbot, MAX_QPATH)) break;
		...
		if (!ReadSpace(fp, filename, line)) break;
		//skin
		if (!ReadString(fp, filename, line, tmpbot.skin, MAX_QPATH)) break;
		if (!ReadSpace(fp, filename, line)) break;
		//charfile
		if (!ReadString(fp, filename, line, tmpbot.charfile, MAX_QPATH)) break;
		if (!ReadSpace(fp, filename, line)) break;
		//charname
		if (!ReadString(fp, filename, line, tmpbot.charname, MAX_QPATH)) break;
		//
...
// Changes Globals:		-
//========================================================================
static void LoadBots(void) {
	bot_t *bot;
	char path[MAX_QPATH], botfile[MAX_QPATH], filespec[MAX_QPATH];
	cvar_t *gamedir;
#if defined(WIN32)|defined(_WIN32)
	...
	strcpy(path, ".");
	AppendPathSeperator(path, MAX_QPATH);
...
void CheckForNewBotFile(void) {
	cvar_t *botfile = gi.cvar("botfile", "bots.cfg", 0);
