...
bool ClientConnect(gentity_t *ent, char *userinfo);
void ClientDisconnect(gentity_t *ent);

typedef struct queuedbot_s {
	int count;
	char library[MAX_QPATH];
...
// Changes Globals:		-
//===========================================================================
static gentity_t *G_SpawnClient(void) {
	int i;
	gentity_t *cl_ent;
	...
	for (i = game.maxclients - 1; i >= 0; i--) {
		...
		if (!cl_ent->inuse) {
			...
			G_InitEdict(cl_ent);
			cl_ent->client = &game.clients[cl_ent - g_entities - 1];
			return cl_ent;
		} //end if
	} //end for
	return NULL;
...
// Changes Globals:		-
//===========================================================================
static void G_FreeClientEdict(gentity_t *ent) {
	ent->s.modelindex = 0;
	ent->solid = SOLID_NOT;
	ent->inuse = false;
...
//								botglobals.numbots
//===========================================================================
static void BotBecome(gentity_t *ent, bot_library_t *lib) {
	bot_state_t *bs;
...
void BotDestroy(gentity_t *bot) {
	...
	bot->client = &game.clients[bot - g_entities - 1];
...
//								botglobals.numbots
//===========================================================================
static gentity_t *BotCreate(char *userinfo, bot_library_t *lib) {
	gentity_t *ent;
	bot_state_t *bs;
...
//===========================================================================
// move the given bot from it's current client gentity_t to a free one
...
// Changes Globals:		botglobals.botstates
//===========================================================================
bool BotMoveToFreeClientEdict(gentity_t *bot) {
	gentity_t *newcl;
	...
	memcpy(newcl, bot, sizeof(gentity_t));
	//copy the contents of the g_client_t structure
	memcpy(newcl->client, bot->client, sizeof(gclient_t));
	//copy bot state
	bs = &botglobals.botstates[bot - g_entities - 1];
	newbs = &botglobals.botstates[newcl - g_entities - 1];
	...
	memset(bot, 0, sizeof(gentity_t));
	//initialize edict
	G_InitEdict(bot);
	//set pointer to g_client structure
	bot->client = &game.clients[bot - g_entities - 1];
	//remove client userinfo
	playernum = bot - g_entities - 1;
...
// Changes Globals:		-
//===========================================================================
static void ShowLoadImage(gentity_t *ent) {
...
static void RemoveLoadImage(gentity_t *ent) {
	if (!ent->client) return;
	//
...
// Changes Globals:		-
//===========================================================================
void AddBotToQueue(gentity_t *ent, char *library, char *userinfo) {
	queuedbot_t *bot;
...
// Changes Globals:		-
//===========================================================================
static bool ClientNameExists(char *name) {
	int i;
	gentity_t *cl_ent;
...
// Changes Globals:		-
//===========================================================================
void BotAddDeathmatch(gentity_t *ent) {
...
// Changes Globals:		-
//===========================================================================
void BotBecomeDeathmatch(gentity_t *ent) {
	bot_library_t *lib;
...
static void SP_bot(gentity_t *self) {
...
// Changes Globals:		-
//===========================================================================
void BotRemoveDeathmatch(gentity_t *ent) {
	int i;
	gentity_t *cl_ent;
	char *name;
