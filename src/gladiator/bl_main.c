...
// Changes Globals:		-
//===========================================================================
void BotExecuteInput(gentity_t *bot) {
	...
	//To get a higher frequency of the Pmove calls the calls are doubled
...
// Changes Globals:		-
//===========================================================================
static void BotSetPMoveState(gentity_t *bot) {
...
// Changes Globals:		-
//===========================================================================
static bot_library_t *GetBotLibrary(gentity_t *bot) {
	bot_library_t *lib;
...
// Changes Globals:		-
//==========================================================================
int BotLib_BotSetupClient(gentity_t *ent, char *userinfo) {
	char *s;
	bot_library_t *lib;
...
// Changes Globals:		-
//==========================================================================
void BotLib_BotShutdownClient(gentity_t *client) {
	bot_library_t *lib;
...
// Changes Globals:		-
//==========================================================================
void BotLib_BotMoveClient(gentity_t *oldclient, gentity_t *newclient) {
	bot_library_t *lib;
...
// Changes Globals:		-
//==========================================================================
void BotLib_BotClientSettings(gentity_t *ent) {
...
// Changes Globals:		-
//==========================================================================
void BotLib_BotSettings(gentity_t *bot, bot_settings_t *settings) {} //end of the function BotLib_BotSettings
...
// Changes Globals:		-
//==========================================================================
void BotLib_BotUpdateClient(gentity_t *bot) {
...
// Changes Globals:		-
//===========================================================================
void BotLib_BotUpdateEntity(gentity_t *ent) {
...
// Changes Globals:		-
//===========================================================================
void BotLib_BotAddSound(gentity_t *ent, int channel, int soundindex, float volume, float attenuation, float timeofs) {
	bot_library_t *lib;
	int entnum;
...
// Changes Globals:		-
//===========================================================================
void BotLib_BotAI(gentity_t *bot, float thinktime) {
	bot_library_t *lib;
...
// Changes Globals:		-
//===========================================================================
void BotLib_BotConsoleMessage(gentity_t *bot, int type, char *message) {
	bot_library_t *lib;
...
// Changes Globals:		-
//===========================================================================
static bsp_trace_t BotLibImport_Trace(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passent, int contentmask) {
...
static bot_library_t *BotLoadLibrary(char *botlibdir) {
	...
	if (botlibhandle == NULL) {
		...
		if (GetBotAPI == NULL) {
			...
			strncpy(lib->path, botlibdir, MAX_QPATH - 1); //Riv++
			lib->path[MAX_QPATH - 1] = '\0';
...
bot_library_t *BotUseLibrary(char *path) {
	...
	if (access(path, 0x04)) {
		...
		cvar = gi.cvar("basedir", "", 0);
		if (cvar) strncat(botlibdir, cvar->string, MAX_QPATH - 1); //Riv++
		botlibdir[MAX_QPATH - 1] = '\0'; //Riv++
		AppendPathSeperator(botlibdir, MAX_QPATH);
		//user specified game directory
		cvar = gi.cvar("gamedir", "", 0);
		if (cvar) strncat(botlibdir, cvar->string, MAX_QPATH - strlen(botlibdir) - 1); //Riv++
		botlibdir[MAX_QPATH - 1] = '\0'; //Riv++
		AppendPathSeperator(botlibdir, MAX_QPATH);
	} //end if
	//the dll name
	strncat(botlibdir, path, MAX_QPATH - strlen(botlibdir) - 1); //Riv++
	botlibdir[MAX_QPATH - 1] = '\0'; //Riv++
...
// Changes Globals:		-
//===========================================================================
static char *Ptr2PathWithMaxSize(char *path, int size) {
	int length;
	char *ptr, *bestptr;
...
void BotSetup(void) {
	...
	botglobals.botnewinput = (bool *)gi.TagMalloc(game.maxclients * sizeof(bool), TAG_GAME);
