...
#define DF_ENTNUMBER(x)			(x - g_entities)
#define DF_ENTCLIENT(x)			(x - 1 - g_entities)
#define DF_NUMBERENT(x)			(&g_entities[x])
#define DF_CLIENTENT(x)			(&g_entities[x + 1])

//bot library
typedef struct bot_library_s {
	char path[MAX_QPATH];				//path to the library
...
void BotLib_BotClientSettings(gentity_t *client);
void BotLib_BotSettings(gentity_t *bot, bot_settings_t *settings);
